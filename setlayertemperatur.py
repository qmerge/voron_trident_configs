...
#
# example Usage: 
#           ./setLayerTemperatur.py -e 215 -s 260 -t 5 -f tempTower_PETG2.gcode
...
try:
    ...
    currentLayer=10;
    currentTemp=int(args.startTemp)
    currentLayerStr=layerList.pop(0)

    for LINE in gcodeInput:
        gcodeOutput.write(LINE)
        myLine=str(LINE)
        if(myLine.find(";LAYER=",0) != -1 ):
            if currentLayerStr != "" and (myLine.find(currentLayerStr,0) != -1 ):
                #print("LINE:            \"{}\"".format(myLine))
